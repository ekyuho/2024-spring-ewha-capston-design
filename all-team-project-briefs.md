...
|만들고자 하는 것|각 대학마다 존재하는 ‘청각장애 학생 도우미’의 역할을 AI로 대신하는 전공 맞춤형 실시간 자막 & 수업 요약본 제공 프로그램<br><br>1) 수업 중 <br>- 학습자가 사전에 수업 자료를 업로드하면, 그 자료를 바탕으로 강의의 주제와 주요 용어를 미리 파악하여 학습자 맞춤형으로 인식률이 향상된 실시간 스크립트를 제공한다. 공식, 그래프나 코드 등을 ‘일반 대화’로 인식하여 구어체로 변환하는 것이 아니라, 사전에 학습한 수업 자료를 바탕으로 ‘전문 지식’으로 인식하여 적절한 공식, 그래프나 그림 등을 추가한 고급 자막으로 번역한다.<br><br>
2) 수업 후<br>- 수업 중 실시간으로 생성한 스크립트가 수업 자료의 어느 부분과 매치되는지 찾아, 생성된 자막과 수업 자료의 위치를 동기화한 ‘스크립트+수업 자료’ 통합형 학습 자료를 제공한다.<br>- 교수님께서 중요하다고 말씀하신 ‘강조 내용’, 구두로 말씀하신 휴강, 강의실 변경, 과제 안내 등의 ‘공지사항’을 요약하여 제공한다.|
...
|만들고자 하는 것|분실자의 분실물 정보를 키워드로 입력받아 몽타주를 활용하여 그에 맞는 이미지를 생성하는 서비스입니다. 
이후에는 Text-to-Image 모델(Dall-E)을 사용하여 몽타주를 생성, 분실자가 살펴봐야 할 정보를 시각화하여 제공합니다. 
이를 통해 사용자가 눈여겨봐야할 정보를 필터링하여 제공함으로써 분실물을 찾는 과정이나 웹 서핑 피로도를 줄여주는 것을 목표로 하는 생성형 AI 검색 웹서비스입니다.|
...
|Pain Point|- 분실물을 찾거나 분실한 경우, 현재는 수많은 데이터를 본인이 일일히 검색해야하므로 상당한 시간이 소요되고, 번거롭습니다.
- 분실자는 분실물에 대한 단편적 정보를 가지고 있기 때문에 분실물의 생김새나 잃어버린 시간, 위치 등을 정확히 기억하지 못하는 경우가 많기에 수 백만 건의 분실물 데이터 속에서 자신의 분실물을 찾지 못하여 발생하는 시간적, 비용적인 낭비가 상당할 것입니다.
- 경찰청 유실물 종합관리시스템(https://www.lost112.go.kr/find/findList.do)에서 공공기관에 보관된 유실물 정보를 제공하고 있지만, 올라오는 물건의 정보가 일관성이 없어 검색이 불편하고 어렵습니다.|
